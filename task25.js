...
*/ 
const awards = ["🏆", "⭐", "💎", "🥇", "👑"];
const randomAwards = (awards) => [...awards][Math.floor(Math.random()*awards.length)]

function getHosts(data){
   return data.reduce((total, cur) => [...total, ...cur.hosts], [])
}

function assignAwards(data){   
  //return getHosts(data).map(el => `${randomAwards(awards)} ${el}`)
  return getHosts(data)
  .map(el => `${awards[Math.floor(Math.random()*awards.length)]} ${el}`)
}
