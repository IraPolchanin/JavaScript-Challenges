...
function createDescriptionsFor(data) {
  return data.map(podcast => {
    ...
    const startHost = hosts.length>1 && hosts.slice(0, -1).join(', ');
    const endHost = hosts.slice(-1).join('')
    const hostsStr = `${startHost && startHost} ${startHost && 'and'} ${endHost}`.trim()

    return {
      ...podcast,
      description: `${title} is a ${duration} minute ${genre} podcast hosted by ${hostsStr}`,
    }
  })
}
